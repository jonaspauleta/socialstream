...
function hasAccountForProvider(provider) {
    return inertiaProps.value.socialstream.connectedAccounts.filter((account) => account.provider === provider).length > 0;
}

function getAccountForProvider(provider) {
    if (hasAccountForProvider(provider)) {
        const tes = inertiaProps.value.socialstream.connectedAccounts.filter((account) => account.provider === provider).shift();
        return tes;
    }
    return undefined;
}
...
<template>
    <ActionSection>
        ...
        <template #description> Manage and remove your connected accounts. </template>

        <template #content>
            <h3 class="text-lg font-medium text-gray-900" v-if="$page.props.socialstream.connectedAccounts.length === 0">You have no connected accounts.</h3>
            <h3 class="text-lg font-medium text-gray-900" v-else>Your connected accounts.</h3>

            <div class="mt-3 ax-w-xl text-sm text-gray-600">
                You are free to connect any social accounts to your profile and may remove any connected accounts at any time. If you feel any of your connected accounts have been compromised, you
            ...
            <div class="mt-5 space-y-6">
                <div v-for="provider in $page.props.socialstream.providers" :key="provider">
                    <ConnectedAccount :provider="provider" :created-at="hasAccountForProvider(provider) ? getAccountForProvider(provider).created_at : null">
                        <template #action>
            ...
            <DialogModal :show="confirmingRemove" @close="confirmingRemove = false">
                <template #title> Remove Connected Account </template>

                <template #content> Please confirm your removal of this account - this action cannot be undone. </template>

                <template #footer>
                    <SecondaryButton @click="confirmingRemove = false"> Nevermind </SecondaryButton>

                    <PrimaryButton class="ml-2" @click="removeConnectedAccount(accountId)" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
